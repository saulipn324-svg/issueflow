...
export default function Home() {
  ...
  const [authReady,setAuthReady] = useState(false); const [username,setUsername] = useState(''); const [password,setPassword] = useState('');
  useEffect(()=>{ if(!mode)return; let active=true;
    if(mode==='demo'){setAuthReady(true);return;}
    api<Session>('/auth/me').then(user=>{if(active)setSession(user);}).catch(()=>{}).finally(()=>{if(active)setAuthReady(true);});
    return ()=>{active=false;};
  },[mode]);
  useEffect(()=>{const expired=()=>{setSession(null);setSelected(null);setFormOpen(false);setConfirm(null);setResult(blankPage);setStats(blankStats);};window.addEventListener('session-expired',expired);return()=>window.removeEventListener('session-expired',expired);},[]);
  async function login(event:React.FormEvent){event.preventDefault();setBusy(true);setError('');try{
    await api<void>('/auth/login',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({username,password}).toString()});
    setPassword('');setSession(await api<Session>('/auth/me'));
  }catch(e){setError(message(e));setPassword('');}finally{setBusy(false);}}
  async function logout(){setBusy(true);setError('');try{if(mode==='api')await api<void>('/auth/logout',{method:'POST'});setSession(null);setSelected(null);setFormOpen(false);setConfirm(null);setResult(blankPage);setStats(blankStats);setNotice('');}catch(e){setError(message(e));}finally{setBusy(false);}}
  ...
  useEffect(() => {
    ...
    return () => abort.abort();
  }, [mode, session, debounced, status, priority, page, revision]);
  ...
  const readyRef = useRef(false); readyRef.current = !!mode && !!session && !busy;
  useEffect(() => {
  ...
  async function confirmAction() {
    ...
    try {
      if (session?.role !== 'ADMIN') throw new Error('Solo el administrador puede eliminar o restaurar datos.');
  ...
  if (!session) return <main className="login-shell"><section className="login-card"><a className="login-brand" href="/">◈ issueflow</a><p className="eyebrow">TU EQUIPO. UN MISMO ESPACIO.</p><h1>Bienvenido de nuevo.</h1><p className="muted">Inicia sesión para dar seguimiento a tus incidencias.</p>{!mode || !authReady ? <p role="status">Preparando el espacio…</p> : mode === 'demo' ? <><div className="notice">Demo interactiva con datos ficticios. Elige un rol para explorar; no necesitas contraseña.</div><div className="login-roles"><Button onClick={()=>{setSession({username:'Administrador demo',role:'ADMIN'});setError('');}}>Probar como administrador</Button><Button variant="outline" onClick={()=>{setSession({username:'Usuario demo',role:'USER'});setError('');}}>Probar como usuario</Button></div><p className="field-hint">Ambos pueden consultar, crear y editar. Solo el administrador puede eliminar incidencias y restaurar ejemplos. Esta simulación no protege datos reales.</p></> : <form onSubmit={login}><div className="field"><label htmlFor="username">Usuario</label><Input id="username" autoComplete="username" required value={username} onChange={e=>setUsername(e.target.value)} disabled={busy}/></div><div className="field"><label htmlFor="password">Contraseña</label><Input id="password" type="password" autoComplete="current-password" required value={password} onChange={e=>setPassword(e.target.value)} disabled={busy}/></div><Button type="submit" disabled={busy}>{busy ? 'Iniciando sesión…' : 'Iniciar sesión'}</Button></form>}{error && <p role="alert" className="error-text">{error}</p>}<a className="doc-link" href="/guia.html">Guía y permisos del proyecto ↗</a></section></main>;
  return <div className="shell">
    ...
    <main className="workspace"><header className="topbar"><span>Mi espacio / <strong>Incidencias</strong></span><div className="session-bar"><span>{session.username} · {session.role === 'ADMIN' ? 'Administrador' : 'Usuario'}</span><Button variant="outline" size="sm" disabled={busy} onClick={logout}>Cerrar sesión</Button></div><span className="mode-label">{mode === 'api' ? 'CONECTADO A SPRING BOOT' : mode === 'demo' ? 'DEMO LOCAL' : 'CONECTANDO…'}</span></header>
